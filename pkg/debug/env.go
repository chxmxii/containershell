...
// EnvOutput returns the environment variables of the container as a string.
func EnvOutput(ctx context.Context, rt runtime.Runtime, containerID string) (string, error) {
	// Try exec first
	stdout, _, exitCode, err := rt.ExecSync(ctx, containerID, []string{"env"}, 10)
	if err == nil && exitCode == 0 {
